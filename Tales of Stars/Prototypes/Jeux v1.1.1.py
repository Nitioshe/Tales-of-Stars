...
class CraftingSystem:
  ...
  def craft(self, recipe_name, inventory):
    if recipe_name in self.receipes:
      recipe = self.receipes[recipe_name]
      ingredients_available = True
      for ingredient, quantity in recipe.items():
        if ingredient not in inventory.items() or inventory.items[ingredient] < quantity:
...
def load_game():
  try:
    ...
    player.__dict__.update(
        data["player"]
    )  # Mettre à jour les attributs du joueur avec les données chargées
...
def main():
  ...
  if training_flag == 0:
    ...
    time.sleep(5)
    monster = Monster("Puppet", 15, 0, 10)
  ...
  if forest_flag == 0:
    ...
    inventory = {"Slime Gall": 1, "Water": 2, "Bottle": 1}
