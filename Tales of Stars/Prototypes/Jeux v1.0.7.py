...
def load_game():
  try:
    ...
    combat_count = data.get("combat_count", 0)
    story_progress = data.get("story_progress", 0)
    print("Game loaded successfully.")
    return player, combat_count, story_progress
...
def main():
  ...
  if load_option == "yes":
    player, combat_count, story_progress = load_game()
    if player is None:
  ...
  while True:
    if intro_flag:
      ...
      mysterious = Image.open('Images\\mysterious.png')
      mysterious.show()
      intro_flag = False
